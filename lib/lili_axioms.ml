...
let axiom_K = Lambda (Bind ("x", Type_atom "'A"), Var "x")
...
let s_axiom_K = "(lambda (x) (lambda (y) x))" |> Sexp.of_string |> parse_term

let s_axiom_S =
  "(lambda (x) (lambda (y) (lambda (z) ((x z) (y z)))))" |> Sexp.of_string
  |> parse_term

let s_axiom_I = "(lambda (x) x)" |> Sexp.of_string |> parse_term

let s_test = "(lambda (x) (lambda (y) (y x)))" |> Sexp.of_string |> parse_term
...
let rec ft t =
  match t with
  | Var x -> Var x
  | Application (t1, t2) -> Application (ft t1, ft t2)
  | Lambda (Bind (x, _), Var y) when x = y -> Var "I"
  | Lambda (Bind (x, _), t1) when nifv x t1 -> Application (Var "K", ft t1)
...
let k =
  ft s_test
  |> (fun x -> subst x "I" s_axiom_I)
  |> (fun x -> subst x "K" s_axiom_K)
  |> (fun x -> subst x "S" s_axiom_S)
  |> untyped_beta_reduction
