...
let make_equation l = List.map (fun (a, b) -> (make_type_var a, b)) l

let rec substitute a b t =
  match t with
  | Var_atom x when x = a -> Var_fixed b
  | Var_arrow (x, y) -> Var_arrow (substitute a b x, substitute a b y)
  | _ -> t

let substitute_all a b lt = List.map (fun (x, y) -> (substitute a b x, y)) lt
...
let unify l =
  let rec step l sol =
    ...
    | (Var_arrow _, Type_atom _) :: _ ->
        ...
        None
    | (Var_fixed x, y) :: rest ->
        if x = y then step rest sol else (* print_endline "clash_2"; *)
                                      None
    | [] -> Some sol
  in
  step l []
...
let rec apply_unifier u t =
  match t with
  | Type_atom x -> ( match List.assoc_opt x u with Some y -> y | None -> t )
  | Type_arrow (a, b) -> Type_arrow (apply_unifier u a, apply_unifier u b)
...
let _ =
  let t1 =
    Type_arrow (Type_arrow (Type_atom "A", Type_atom "B"), Type_atom "A")
    |> make_type_var
  in
  let t2 =
    Type_arrow (Type_arrow (Type_atom "p", Type_atom "q"), Type_atom "r")
  in
  unify [ (t1, t2) ]
